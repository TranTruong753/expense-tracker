        ...
        if (validateForm()) {
            ...
            try {
                ...
                if (err.response?.status === 400) {
                    ...
                    newErrors.amount = 'số tiền trong tài khoản không đủ!';
                    setErrors(newErrors);
                    return notifications.show('số tiền trong tài khoản không đủ!', { severity: 'error' })
                }
                            ...
                            >
                                {listBank && listBank.map((bank) => (
                                    <MenuItem key={bank.id} value={bank.id}>
                                        {bank.bankName} - số dư: {formatCurrency(bank.balance)}
                                    </MenuItem>
                                ))}
                            </Select>
